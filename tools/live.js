#!/usr/bin/env node
/* =============================================================================
   live.js — turns ANY public website into real dashboard data. Zero keys, zero
   cost, zero accounts. Node 18+ only (uses global fetch).
   ...
   What it collects
     • your own crawl : titles, metas, H1s, ALT, word counts, links in/out, status,
     ...
     • PageSpeed Insights + CrUX field data — only if you pass PSI_KEY (the endpoint
       rate-limits anonymous calls, so we do not pretend it always works)
     • Google Search Console CSV exports, if you have them — that is the ONLY honest
       source of impressions / clicks / positions. We never model those.

   Output: <out>/site.json in the exact shape index.html reads as window.DATA.
   Then:   python3 tools/bake.py index.html <out>/site.json
   ============================================================================= */
'use strict';
...
const list = k => CLI.reduce((acc, a, i) => (a === '--' + k ? acc.concat(A1(i)) : acc), []);
const A1 = i => CLI[i + 1];
const listFiles = k => CLI.reduce((acc, a, i) => (a === '--' + k ? acc.concat(CLI[i + 1]) : acc), []);

const DEFAULTS = {
  maxPages: 180, depth: 3, slow: 250, out: 'out/live', psiKey: process.env.PSI_KEY || '', gscCsv: [],
  ua: process.env.AUDIT_UA || 'Mozilla/5.0 (compatible; ViralyticsAudit/1.0; +read-only SEO audit)',
};
...
async function crawl() {
  ...
  while (q.length && n < CFG.maxPages) {
    ...
    if (n % 25 === 0) process.stdout.write(`\r  crawled ${n}/${CFG.maxPages}  (queue ${q.length}, ${(n * 1)}/${Math.max(1, ((Date.now() - t0) / 1000)).toFixed(0)}s)  `);
    const w = CFG.slow - r.ms; if (w > 0) await sleep(w);
  }
  if (n >= 25) process.stdout.write('\r');
...
async function psi(pages) {
  ...
  for (const p of pick) {
    ...
    await sleep(900);
    process.stdout.write(`\r  PageSpeed ${Object.keys(out).length}/${pick.length}   `);
  }
...
/* run history: the dashboard POSTs its snapshots to out/<domain>/history.json (go.js);
   carrying them in the payload means the next baked file remembers its own runs. */
function loadHistory(domain) {
  const root = path.join(__dirname, '..');
  for (const dir of [path.join(root, 'out', domain), CFG.out]) {
    try {
...
async function collect(target, opts) {
  const quiet = !!(opts && opts.quiet); const log = quiet ? () => {} : console.log.bind(console);
  ...
  if (home.finalUrl) { try { const fo = new URL(home.finalUrl); if (!opt('origin') && fo.origin !== ORIGIN) { ORIGIN = fo.origin; console.log('  following redirect → ' + ORIGIN) } } catch (e) {} }
  ...
  log(' crawling …'); const { pages, crawled, seconds } = await crawl();
  log(' PageSpeed / CrUX …'); const perf = await psi(pages);
  ...
  let keywords = [];
  for (const f of (CFG.gscCsv || [])) { log(` GSC csv ${f} …`); try { keywords.push(...parseGscCsv(f)) } catch (e) { log('  csv skipped: ' + e.message) } }
  keywords = keywords.slice(0, 1000).map(k => ({ q: k.q, imp: k.imp, clicks: k.clicks, pos: k.pos, page: k.page, intent: 'informational', fit: 'blog' }));

  const data = {
    generatedAt: new Date().toISOString(), live: true, demo: false, engine: 'live.js v2',
    ...
    keywords,
    history: loadHistory(new URL(ORIGIN).hostname),
    competitors: [],
  ...
  const f = path.join(CFG.out, 'site.json');
  ...
  console.log(`   sitemap: ${sm.found.map(x => x.url.split('/').pop() + '(' + x.urls + ')').join(', ') || 'none found'} · declared ${sm.declaredCount} URLs`);
  console.log(`   CWV: ${perf.skipped ? perf.skipped : 'measured on ' + Object.keys(perf).length + ' pages'}`);
  console.log(`   wrote ${f}  (${(fs.statSync(f).size / 1024).toFixed(1)} KB)`);
  console.log(`\n next: python3 tools/bake.py ${f} --html index.html --out live-<domain>.html`);
...
async function quick(target, { maxPages = 12, depth = 1 } = {}) {
  const d = await collect(target, { maxPages, depth, quiet: true, slow: 120 });
...
async function serpSample(target, keywords, { throttleMs = 1400 } = {}) {
...
module.exports = { collect, quick, serpSample, parseGscCsv };
...
if (require.main === module) (async () => {
  ...
  const d = await collect(target, { maxPages: +opt('pages', 180), depth: +opt('depth', 3), slow: +opt('delay', 250),
    out: opt('out', 'out/live'), gscCsv: listFiles('gsc-csv'), serp });
  if (serp.length) { console.log('\n SERP sample · DuckDuckGo position (NOT Google — a directional check):');
    ...
    fs.mkdirSync(opt('out', 'out/live'), { recursive: true });
    fs.appendFileSync(path.join(opt('out', 'out/live'), 'serp-sample.json'), JSON.stringify(serp, null, 1)); }
