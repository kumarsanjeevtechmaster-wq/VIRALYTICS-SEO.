#!/usr/bin/env node
/* report-md.js — turns a live-baked dashboard + its payload into a client-ready markdown
   action plan.  node tools/report-md.js live-site.com.html out/site LIVE-AUDIT-site.md */
'use strict';
...
global.document = { createElement: mk, createElementNS: mk, createTextNode: t => ({ nodeType: 3, textContent: t }), querySelector() { const e = mk('q'); e.append = () => {}; return e }, querySelectorAll: () => [], addEventListener(t, f) { listeners[t] = f }, title: '', body: mk('b'), documentElement: mk('h'), head: mk('h2'), getElementById: () => mk('id') };
global.window = global; global.self = global; global.addEventListener = (t, f) => { listeners[t] = f };
const mem = {}; global.localStorage = { getItem: k => mem[k] ?? null, setItem: (k, v) => { mem[k] = String(v) }, removeItem: () => { } };
global.navigator = { userAgent: 'report-md', clipboard: {} };
/* the injected UI registers timers + a localhost ping — neutralise both here */
global.setInterval = () => 0; global.setTimeout = () => 0; global.location = { protocol: 'file:', href: 'f', origin: 'n' };
global.fetch = async () => { throw 0 }; global.scrollTo = () => {}; global.prompt = () => '';
global.getComputedStyle = () => ({ getPropertyValue: () => '' }); global.requestAnimationFrame = f => f();
global.matchMedia = () => ({ matches: false }); global.Blob = class { }; global.URL.createObjectURL = () => 'x'; global.URL.revokeObjectURL = () => { };
if (PAYLOAD && fs.existsSync(PAYLOAD)) window.DATA = JSON.parse(fs.readFileSync(PAYLOAD, 'utf8'));
blocks.forEach(b => vm.runInThisContext(b));

const d = window.DATA || {}; const st = d.stats || {}; const sm = d.sitemaps || {};
...
p('# Live SEO audit — ' + (d.site ? d.site.domain : path.basename(FILE)) );
p('');
...
p('python3 tools/bake.py out/live/site.json --html index.html --out live.html');
...
console.log('wrote ' + OUT + ' · ' + L.length + ' lines · ' + pages.length + ' pages · score ' + sc.score + '/100 · ' + iss.length + ' groups · ' + n + ' priority fixes');
